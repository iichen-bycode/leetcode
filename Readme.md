...
> 划分型回溯（不使用path累加，先拆分在回到子集回溯的 选与不选 如：1593）
> 需要使用上一个元素值继续处理 比如：2698（中的处理值：1298 对于9  有 9 +  8 再有9 * 10 + 8）

> 排列回溯：不使用startIndex了，元素可以重复使用 46
> 
> 初始：[1,2,3,4],以1开始下一次[2,3,4]，以2开始下一次[1,3,4]所以就是需要标记去除已经选择的，去选择剩余的

> 组合回溯：还是使用startIndex 77
...
> 公共祖先：对于当前节点判断左右子树是否包含目标值 左右子树返回的都不是null 说明目标分别在左右，则结果就是当前节点，如果左为null,右不为null,则返回右，反之返回左
> 235 236
...
> KMP next[i] 前i个元素相同的前后缀数目（可以想象每次不匹配折半移动）
> 
```java
...
> 扩展KMP-Z函数 （Z[i] 表示从i开始字串与原串的最长公共前缀）
> 就是s[i...]的最长前缀Len与原串的s[0...Len]的Len相同 所以后续计算可以使用之前的计算结果
```java
public int[] getZArray(String s) {
    int n = s.length();
    int[] z = new int[n];
    int l = 0, r = 0;  // 当前 Z-box 区间 [l, r]
    for (int i = 1; i < n; i++) {
        // 在区间内直接更新
        if (i <= r) {
...
```
> 
> Z-Box求解Z函数，定义区间l,r 表示包含 i 的最大前缀，r能达到的最右边界值

#### 回文串

##### 动态规划

> dp[i][j] 表示 字符串i到j是否为回文字符串
...
public String longestPalindrome(String s) {
     ...
     int start = (centerIndex - maxLen) / 2; // 原始串中的位置

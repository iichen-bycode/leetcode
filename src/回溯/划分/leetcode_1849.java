...
public class leetcode_1849 {
    public static void main(String[] args) {
        ...
        ans = false;
        System.out.println(splitString("9465072333777578147"));
    }
    ...
    private static void backTracking(String s, int start, ArrayList<String> list) {
        if (start == s.length()) {
            if (list.size() > 1) {
//                Utils.printListS(list);
                ans = true;
            }
            return;
        }
        for (int i = start; i < s.length(); i++) {
            ...
            if (list.isEmpty() || Double.parseDouble(list.get(list.size() - 1)) - Double.parseDouble(sub) == 1) {
                list.add(sub);
